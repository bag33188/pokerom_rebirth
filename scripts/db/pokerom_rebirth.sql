...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 12, 2022 at 11:11 AM
...
CREATE DEFINER=`bag33188`@`%` PROCEDURE `FindRomsWithNoGame` ()  READS SQL DATA BEGIN
	SELECT `id`, `rom_name`, `rom_type`, /* IF (0 = FALSE, 'false', 'true') AS */ `has_game`, `game_id`
    FROM `roms`
    WHERE `has_game` = FALSE AND `game_id` IS NULL
	...
	START TRANSACTION;
  	UPDATE `roms`
  		SET `file_id` = `ROM_FILE_ID`,
      		`rom_size` = CEIL(`ROM_FILE_SIZE` / 1024), -- get Kibibytes value from bytes
      		`has_file` = TRUE
  		WHERE `id` = `ROM_ID`;
    IF `_rollback` THEN
        ROLLBACK;
...
CREATE DEFINER=`bag33188`@`%` FUNCTION `SPLIT_STRING` (`STR_VAL` VARCHAR(256), `SEPARATOR` VARCHAR(1) CHARSET utf8, `POSITION` SMALLINT) RETURNS VARCHAR(256) CHARSET utf8mb4 DETERMINISTIC COMMENT 'splits a string based on delimiter ' BEGIN
...
--
-- Creation: Jul 06, 2022 at 02:20 AM
-- Last update: Jul 12, 2022 at 08:41 AM
--
...
--
-- Creation: Jul 06, 2022 at 02:20 AM
-- Last update: Jul 12, 2022 at 08:47 AM
--
...
('hpmcgjqShvVIvunNuIYjDkApYvKmwwHd9s0NXjrk', 1, '127.0.0.1', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36', '', 1657615651);
